...
void PERFORM_BEHAVIORS(){
	
	for(int i = 0; i < storedEntities_count;i++){
		
		if ( ! IS_ACTIVE(arr_entities[i]) ){
			continue;
		}

		if (1 == arr_entities[i] -> markedForDeletion){
			continue;
		}
		
		if (0 == arr_entities[i]->ptrBehaviorFN){
			continue;
		}

		if (NULL == arr_entities[i]->ptrBehaviorFN){
			continue;
		}
		
		arr_entities[i]->ptrBehaviorFN(arr_entities[i]);
	}
	return;
}
